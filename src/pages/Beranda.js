...
function Beranda() {
  ...
  // ✅ CRA ENV yang konsisten (anti balik localhost)
  const API_BASE_URL =
  ...
  const fetchMotors = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`${API_BASE_URL}/api/motors`);
  ...
  const handleSimpanMotor = async (e) => {
    e.preventDefault();
    const dataToSubmit = { ...formData, stok: parseInt(formData.stok, 10) };
    ...
    try {
      ...
      if (isEditing) {
        // update (Laravel method spoofing)
        await axios.post(
          `${API_BASE_URL}/api/admin/motors/${editId}`,
  ...
  const handleHapusMotor = async (id) => {
    if (window.confirm("Hapus motor ini secara permanen?")) {
      try {
        ...
        await axios.delete(`${API_BASE_URL}/api/admin/motors/${id}`, config);
        alert("Data berhasil dihapus!");
        fetchMotors();
      } catch (err) {
        console.error("Hapus motor error:", err);
    ...
    >
      <nav style={styles.nav}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "20px" }}>
          ...
          <div style={{ position: "relative" }}>
            <button onClick={() => setShowMenu(!showMenu)} style={styles.btnMenu}>
              ⋮
            </button>
            ...
            <AnimatePresence>
              {showMenu && (
                ...
                >
                  {userRole === "admin" ? (
                    <>
                      <div style={styles.menuItem} onClick={() => navigate("/pesanan-masuk")}>
                        Pesanan Masuk
                      </div>
                      <div style={styles.menuItem} onClick={() => navigate("/stok-barang")}>
                        Stok Barang
                      </div>
                    </>
                  ) : (
                    <>
                      <div style={styles.menuItem} onClick={() => navigate("/pesanan-saya")}>
                        Pesanan Saya
                      </div>
                      <div style={styles.menuItem} onClick={() => navigate("/profil-perusahaan")}>
                        Profil Perusahaan
                      </div>
                    </>
                  )}
      ...
      <div style={styles.banner}>
        <motion.h1
          ...
          style={{ fontSize: "48px", fontWeight: "900", marginBottom: "10px" }}
        >
      ...
      <div style={{ padding: "60px 8%" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "40px" }}>
          <h3 style={{ borderLeft: `5px solid ${theme.primary}`, paddingLeft: "15px", margin: 0, fontSize: "24px" }}>
            KATALOG TERBARU
          </h3>
        ...
        {loading ? (
          <p style={{ textAlign: "center", color: theme.textSub }}>Memuat data motor...</p>
        ) : (
            ...
            {motors.map((item, index) => (
              ...
              >
                ...
                <div style={{ padding: "20px" }}>
                  ...
                  <h3 style={{ color: theme.success, margin: "0 0 20px 0" }}>
                    Rp {Number(item.harga).toLocaleString("id-ID")}
                  </h3>

                  {userRole === "admin" ? (
                    <div style={{ display: "flex", gap: "10px" }}>
                      <button onClick={() => openEditModal(item)} style={{ ...styles.btnSmall, backgroundColor: "#334155" }}>
                        Edit
                      </button>
                      <button
                        onClick={() => handleHapusMotor(item.id)}
                        style={{ ...styles.btnSmall, backgroundColor: "#450a0a", color: "#f87171" }}
                      >
                        Hapus
                      </button>
                    </div>
                  ) : (
                    <button onClick={() => openDetailModal(item)} style={styles.btnPrimary}>
                      Detail Produk
                    </button>
                  )}
                </div>
              ...
              {showForm ? (
                <>
                  ...
                  <form onSubmit={handleSimpanMotor}>
                    ...
                    <input
                      style={styles.input}
                      value={formData.nama_model}
                      onChange={(e) => setFormData({ ...formData, nama_model: e.target.value })}
                      required
                    />
                    ...
                    <div style={{ display: "flex", gap: "10px", marginTop: "10px" }}>
                      ...
                      <button
                        ...
                        style={{ ...styles.btnSmall, flex: 1, backgroundColor: "transparent", border: `1px solid ${theme.border}` }}
                      >
                        Batal
                      </button>
                    </div>
                ...
                <div style={{ display: "flex", gap: "30px", flexWrap: "wrap" }}>
                  <div style={{ flex: "1", minWidth: "300px" }}>
                    <img
                      ...
                      style={{ width: "100%", borderRadius: "12px", backgroundColor: "#fff", padding: "10px" }}
                      alt="motor"
                    />
                  </div>
                  ...
                  <div style={{ flex: "1.2" }}>
                    ...
                    <div style={{ display: "flex", gap: "12px", marginTop: "30px" }}>
                      <button onClick={() => handlePesanMotor(selectedMotor.id)} style={{ ...styles.btnPrimary, flex: 2 }}>
                        Pesan Sekarang
                      </button>
                      <button
                        onClick={() => setShowDetail(false)}
                        style={{ ...styles.btnSmall, flex: 1, backgroundColor: theme.card, border: `1px solid ${theme.border}` }}
                      >
                        Kembali
                      </button>
                    </div>
...
const styles = {
  ...
  menuItem: { padding: "12px 20px", cursor: "pointer", fontSize: "14px", transition: "0.2s", color: theme.textMain },
  banner: {
    background: `linear-gradient(rgba(15, 23, 42, 0.7), rgba(15, 23, 42, 0.9)), url("beranda1.png")`,
    height: "450px",
